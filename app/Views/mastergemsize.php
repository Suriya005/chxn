...
<div class="row " style="padding-left:250px; margin-bottom:15px; margin-top:100px; font-size:25px;">
  ...
  <div class="col-3"><button type="button" class="btnchxcan" style="margin-right:20px; width:150px; ">ยกเลิก</button><button type="button" class="btnchxsave" style="width:150px;" data-bs-toggle="modal" data-bs-target="#exampleModal" id="submit_btn" onclick="insert_form()" disabled>บันทึก</button></div>
</div>
    ...
    <div style="background-color:white; padding-left:30px; ">
      <div class="row" style="padding-top:20px; ">
        <div class="col-4">
          ...
          <input type="text" id="code" name="code" class="form-control mb-3" style="width: 477px;height: 45px;" placeholder="รหัส Code">
          <lebel>ชื่อ</lebel></br>
          <input type="text" id="name" name="name" class="form-control mb-3" style="width: 477px;height: 45px;" placeholder="ชื่อ">
          <lebel>รายละเอียด</lebel></br>
          <textarea class="form-control mb-3" name="detail" id="detail" placeholder="รายละเอียด" style="width:477px; height:128px;"></textarea>
        </div>
        <div class="col-8" id="size-border" style="height: 455px;">
          ...
          <div class="form-check">
            <label class="containerv2" style="  padding-left:40px; margin-top:0px; width: 150px;">เลือกทั้งหมด
              <input type="checkbox">
              <input type="hidden" id="group_lenght" name="group_lenght" value="<?php echo count($gem_group); ?>">
              <span class="checkmarks"></span>
            </label>
          </div>

          <?php
          foreach ($gem_group as $key => $item) {
          ?>
            <div class="form-check">
...
<script>
  ...
  function delect_item(uid) {
    if (confirm('ต้องการลบข้อมูลหรือไม่ ?')) {
      ...
      main_form.action = `master/gem_size/delete/${uid}`;
      main_form.submit();
    }
  }
  ...
  function insert_form() {
    ...
    main_form.action = "<?= base_url('master/gem_size/insert') ?>";
  }

  function edit_form(uid) {
    let main_form = document.getElementById('main_form');
    main_form.action = `master/gem_size/edit/${uid}`;
  }
